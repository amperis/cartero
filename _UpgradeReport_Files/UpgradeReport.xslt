...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
    ...
    <xsl:sort select="@Name" order="ascending"/>
        <h2>
        <xsl:if test="@solution">Solución: <xsl:value-of select="@solution"/></xsl:if>
        <xsl:if test="not(@solution)">Proyecto: <xsl:value-of select="@name"/>
                ...
                <tr class="row">
                    ...
                    <td class="content">
                        <xsl:if test="count(event[@error-level='3'])=1">
                            ...
                            <xsl:if test="@description='Converted'">Convertido</xsl:if>
                            <xsl:if test="@description!='Converted'"><xsl:value-of select="@description"/></xsl:if>
                            </xsl:for-each>
                        </xsl:if>
                        <xsl:if test="count(event[@error-level='3'])!=1 and count(event[@error-level='3' and @description='Converted'])!=0">Convertido
                        </xsl:if>
                    </td>
                ...
                <tr class="collapsed" bgcolor="#ffffff">
                    ...
                    <td colspan="7">
                        <table width="97%" border="1" bordercolor="#dcdcdc" rules="cols" class="issuetable">
                            <tr>
                                <td colspan="7" class="issuetitle" _locID="ConversionIssues">Problemas de conversión - <xsl:value-of select="@name"/>:</td>
                            </tr>
            ...
            <tr valign="top">
                <td class="foot">
                    <xsl:if test="count(source)!=1">
                        <xsl:value-of select="count(source)"/> archivos
                    </xsl:if>
                    <xsl:if test="count(source)=1">
                        1 archivo
                    </xsl:if>
                ...
                <td class="foot">
                    Convertido: <xsl:value-of select="count(source/event[@error-level='3' and @description='Converted'])"/><BR/>
                    Sin convertir<xsl:value-of select="count(source) - count(source/event[@error-level='3' and @description='Converted'])"/>
                </td>
    ...
    <xsl:template match="UpgradeLog">
        <html>
            ...
            <body topmargin="0" leftmargin="0" rightmargin="0" onclick="outliner();">
                ...
                <p><span class="note">
                <b>Tiempo de conversión:</b>  <xsl:value-of select="Properties/Property[@Name='Date']/@Value"/>  <xsl:value-of select="Properties/Property[@Name='Time']/@Value"/><br/>
                </span></p>
                ...
                <table class="note">
                    <tr>
                        <td nowrap="1">
                            <b>Configuración de conversión</b>
                        </td>
                    </tr>
